  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const ordersWithDecrypted = orders.map((order) => ({
        ...order,
        rdpPassword: order.rdpPasswordEncrypted
          ? decrypt(order.rdpPasswordEncrypted)
          : null,
        tier:
          (order.metadata as Record<string, unknown> | null)?.tier ?? "basic",
      }));
      ...
      if (tier === "pro") {
        ...
        if (proInstanceId) {
          try {
            ...
            logger.warn(
              { error, orderId: order.id, proInstanceId },
              "Failed to delete Pro instance — DB will still be marked TERMINATED",
            );
          }
        }
      } else {
        if (order.linodeInstanceId) {
          try {
            ...
            logger.warn(
              ...
              "Failed to delete Linode instance — DB will still be marked TERMINATED",
            );
          }
        }
      }
