...
export default function AppShell({ children }: AppShellProps) {
  ...
  useEffect(() => {
    ...
    api<{ success: boolean; data: Order[] }>("/orders")
      .then(r => setActiveCount(r.data.filter(o => o.status === "ACTIVE").length))
      .catch(() => {});
  }, [isSignedIn]);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] flex">
      <aside className="hidden lg:flex flex-col fixed left-0 top-0 h-full w-[240px] bg-[#0d0d14] border-r border-[rgba(0,240,255,0.08)] z-30">
        <SidebarContent location={location} activeCount={activeCount} user={user} isSignedIn={isSignedIn} />
      </aside>
      ...
      <div className="flex-1 lg:ml-[240px] flex flex-col min-h-screen">
        <header className="lg:hidden flex items-center justify-between px-4 py-3 border-b border-[rgba(0,240,255,0.08)] bg-[#0d0d14] sticky top-0 z-20">
          <button onClick={() => setMobileOpen(true)} className="text-[#a0a0b8] hover:text-white transition-colors p-1">
            <Menu className="w-5 h-5" />
          </button>
          <span className="font-orbitron text-sm font-bold tracking-[2px] text-white uppercase">Cyberise</span>
          {isSignedIn && <UserButton appearance={{ elements: { avatarBox: "w-7 h-7" } }} />}
        </header>
...
function SidebarContent({ location, activeCount, user, isSignedIn }: {
  location: string;
  activeCount: number;
    ...
    <>
      <div className="px-5 py-6 border-b border-[rgba(0,240,255,0.06)]">
        <Link href="/" className="flex items-center gap-2.5 group">
          ...
          <div>
            <div className="font-orbitron text-[0.8rem] font-bold tracking-[2px] text-white uppercase leading-none">Cyberise</div>
            <div className="text-[0.6rem] text-[#a0a0b8] tracking-[1.5px] uppercase mt-0.5">Technology</div>
          </div>
        </Link>
      </div>
          ...
          return (
            <Link key={href} href={href}>
              <div className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all cursor-pointer group ${
                ...
                <Icon className={`w-4 h-4 flex-shrink-0 ${active ? "text-[#00f0ff]" : "text-[#666] group-hover:text-[#a0a0b8]"}`} />
          ...
          <div className="flex items-center gap-3">
            <UserButton
              appearance={{
                elements: {
                  avatarBox: "w-8 h-8",
                  userButtonPopoverCard: "bg-[#12121a] border border-[rgba(0,240,255,0.15)]",
                }
              }}
            />
            <div className="flex-1 min-w-0">
              <p className="text-white text-xs font-medium truncate">{user?.firstName || "User"}</p>
              <p className="text-[#666] text-[10px] truncate">{user?.primaryEmailAddress?.emailAddress}</p>
            </div>
          </div>
        )}
      </div>
